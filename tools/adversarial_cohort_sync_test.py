#!/usr/bin/env python3
"""Search cohort-synchronization disagreements and audit a sealed holdout split."""
...
SOURCE = ROOT / "scheduler_versions" / "v23_cohort_sync_experiment.cpp"
JUDGE = ROOT / "tools" / "local_judge.py"
WORK_DIR = ROOT / "build" / "adversarial-cohort-sync"
MODES = (
    "smooth_amortized",
    "latency_tension",
    "medium_knee",
...
def dpost_curve(rng: random.Random, mode: str) -> list[float]:
    base = math.exp(rng.uniform(math.log(0.15), math.log(8.0)))
    knee = rng.choice([2, 4, 8, 16])
    values: list[float] = []
    for size in BATCH_SIZES:
        if mode == "smooth_amortized":
            scale = 0.75 + 0.25 * size ** rng.uniform(0.35, 0.65)
        elif mode == "latency_tension":
            scale = 0.9 + 0.1 * size ** rng.uniform(0.25, 0.5)
        elif mode == "medium_knee":
            scale = 0.55 + 0.30 * min(size, knee) ** 0.55
            if size > knee:
                scale += rng.uniform(0.6, 1.6) * (size - knee) / knee
        elif mode == "large_batch_cliff":
            scale = 0.6 + 0.22 * size**0.5
            if size >= knee:
                scale *= rng.uniform(2.5, 9.0)
        elif mode == "nonmonotonic":
            scale = 0.65 + 0.28 * size**0.55
            if size in {knee, min(4096, 2 * knee)}:
                scale *= rng.uniform(0.35, 0.7)
        elif mode == "flat_post":
            scale = 1.0 + rng.uniform(0.005, 0.04) * size
        elif mode == "schedule_heavy":
            scale = 0.9 + 0.08 * size**0.65
        else:
            scale = 0.7 + 0.2 * size**rng.uniform(0.4, 0.75)
...
def make_scenario(seed: int, split: str, index: int) -> dict[str, Any]:
    ...
    data["name"] = f"cohort_{split}_{index:03d}_{mode}"
    data["description"] = (
        f"Deterministic cohort-sync adversarial {split} case; mode={mode}, seed={seed}."
    )

    system = data["system"]
    system["K"] = rng.randint(1, 4)
    system["S"] = round(
        rng.uniform(4.0, 14.0) if mode == "schedule_heavy" else rng.uniform(0.05, 3.0),
        9,
    )
    system["latency_in_ms"] = round(rng.uniform(0.001, 0.2), 9)
    system["bandwidth_gbps"] = round(rng.uniform(10.0, 100.0), 9)
    system["bytes_per_token"] = rng.choice([64, 128, 256, 512, 1024, 2048])

    post_values = dpost_curve(rng, mode)
    for row_index, row in enumerate(data["task_times"]):
        row["decode_post"] = post_values[row_index]
        row["decode_pre"] = round(max(0.001, float(row["decode_pre"]) * 0.08), 9)
        row["decode_proc"] = round(max(0.001, float(row["decode_proc"]) * 0.08), 9)
        row["prefill_pre"] = round(max(0.001, float(row["prefill_pre"]) * 0.12), 9)
        row["prefill_proc"] = round(max(0.001, float(row["prefill_proc"]) * 0.08), 9)
        row["prefill_post"] = round(max(0.001, float(row["prefill_post"]) * 0.12), 9)

    requests = data["requests"]
    if mode in {"mixed_waves", "latency_tension"}:
        wave = rng.choice([2, 4, 8])
        gap = rng.uniform(0.02, 2.5)
    ...
    for request_id, request in enumerate(requests):
        if request_id % 5 == 0:
            request["output_length"] = rng.choice([1, 2, 32, 48, 64])
        else:
            request["output_length"] = rng.choice([2, 4, 8, 12, 16, 24])

    request_count = len(requests)
    eligible_sizes = [size for size in BATCH_SIZES if size <= request_count]
    schedule_cost = float(system["S"])
    peak_post_rate = max(
        size / (schedule_cost + post_values[BATCH_SIZES.index(size)])
        for size in eligible_sizes
    )
    singleton_post = schedule_cost + post_values[0]
    if mode == "latency_tension":
        throughput_weight = rng.choice([0.0, 0.1, 0.25])
        slo2 = singleton_post * rng.uniform(0.8, 2.2)
    else:
        throughput_weight = rng.choice([0.25, 0.5, 0.75, 0.95, 1.0])
        slo2 = singleton_post * rng.uniform(1.0, 5.0)
    scoring = data["scoring"]
    scoring["SLO1"] = round(max(float(scoring["SLO1"]), 10_000.0), 9)
    scoring["SLO2"] = round(max(0.001, slo2), 9)
    scoring["tp_base"] = round(max(1e-10, 0.05 * peak_post_rate), 12)
    scoring["tp_UB"] = round(max(2e-10, 0.9 * peak_post_rate), 12)
...
def verify_pool() -> dict[str, Any]:
    ...
    if not manifest_path.is_file():
        raise RuntimeError("scenario pool does not exist; run the search phase first")
...
def evaluate(executable: pathlib.Path, split: str, label: str) -> list[dict[str, Any]]:
    output_path = WORK_DIR / "results" / f"{label}-{split}.json"
...
def dpost_signature(row: dict[str, Any]) -> list[tuple[float, tuple[int, ...]]]:
    signature = []
    for frame in row.get("assignment_trace", []):
        for assignment in frame["assignments"]:
            if assignment["family"] == "D" and assignment["step"] == "POST":
                signature.append(
                    (round(float(frame["time"]), 9), tuple(assignment["members"]))
                )
    return signature
...
def compare(
        ...
        comparisons.append(
            {
                ...
                "dpost_disagreement": dpost_signature(candidate) != dpost_signature(baseline),
            }
        )
    ...
    return {
        ...
        "dpost_disagreements": sum(row["dpost_disagreement"] for row in comparisons),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--search-count", type=int, default=256)
    parser.add_argument("--holdout-count", type=int, default=128)
    parser.add_argument("--reference-level", type=int, default=20)
    parser.add_argument("--candidate-level", type=int, default=23)
    parser.add_argument("--work-dir", type=pathlib.Path, default=WORK_DIR)
    parser.add_argument("--search-seed-base", type=int, default=225_230_000)
    parser.add_argument("--holdout-seed-base", type=int, default=225_239_999)
    parser.add_argument(
        "--json-out",
        type=pathlib.Path,
        default=None,
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "caveat": (
            "The search split is intentionally adversarial and may guide implementation. "
            "The separately seeded holdout split must be evaluated only after the candidate is fixed. "
            "Neither split estimates the official hidden workload distribution."
        ),
        "splits": {},
    }
    ...
    for split in phases:
        ...
        print(
            ...
            f"DPOST disagreements={summary['dpost_disagreements']}"
        )
