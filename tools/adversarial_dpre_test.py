#!/usr/bin/env python3
"""Search D PRE-to-D POST disagreements and audit a separately seeded split."""
...
SOURCE = ROOT / "scheduler_versions" / "v21_dpre_experiment.cpp"
JUDGE = ROOT / "tools" / "local_judge.py"
WORK_DIR = ROOT / "build" / "adversarial-dpre"
MODES = (
    "finite_remainder",
    "pipeline_shift",
    "smooth_amortized",
...
def make_scenario(seed: int, split: str, index: int) -> dict[str, Any]:
    ...
    data["name"] = f"dpre_{split}_{index:03d}_{mode}"
    data["description"] = (
        f"Deterministic D PRE adversarial {split} case; mode={mode}, seed={seed}."
    )
    ...
    pre_values = stage_curve(rng, mode, "pre")
    proc_values = stage_curve(rng, mode, "proc")
    post_values = stage_curve(rng, mode, "post")
    if mode in {"finite_remainder", "pipeline_shift"}:
        knee = rng.choice([2, 4, 8])
        schedule_cost = float(system["S"])
        knee_per_member = 2.0 * schedule_cost / knee
        for row_index, size in enumerate(BATCH_SIZES):
            if size <= knee:
                pre_values[row_index] = round(
                    schedule_cost * (0.5 + 0.5 * size / knee), 9
                )
            else:
                pre_values[row_index] = round(
                    1.05 * knee_per_member * size - schedule_cost, 9
                )
            if mode == "pipeline_shift":
                proc_values[row_index] = round(
                    schedule_cost * (2.0 + 1.6 * math.sqrt(size)), 9
                )
                post_values[row_index] = round(
                    schedule_cost * (1.4 + 1.1 * math.sqrt(size)), 9
                )
            else:
                proc_values[row_index] = round(
                    0.1 * schedule_cost * (0.8 + 0.2 * math.sqrt(size)), 9
                )
                post_values[row_index] = round(
                    0.08 * schedule_cost * (0.8 + 0.2 * math.sqrt(size)), 9
                )
    for row_index, row in enumerate(data["task_times"]):
        row["decode_pre"] = pre_values[row_index]
        row["decode_proc"] = proc_values[row_index]
        row["decode_post"] = post_values[row_index]
        row["prefill_pre"] = round(20.0 + 0.02 * row_index, 9)
        row["prefill_proc"] = round(0.01 + 0.001 * row_index, 9)
        row["prefill_post"] = round(0.01 + 0.001 * row_index, 9)

    requests = data["requests"]
    if mode in {"mixed_waves", "link_queue"}:
    ...
    schedule_cost = float(system["S"])
    pre_rate = max(
        size / (schedule_cost + pre_values[BATCH_SIZES.index(size)])
    ...
    singleton_path = (
        3.0 * schedule_cost + pre_values[0] + proc_values[0] + post_values[0]
        + 2.0 * float(system["latency_in_ms"])
    )
    throughput_weight = rng.choice([0.95, 1.0])
...
def dpre_signature(row: dict[str, Any]) -> list[tuple[float, str, tuple[int, ...]]]:
    signature = []
    for frame in row.get("assignment_trace", []):
        for assignment in frame["assignments"]:
            if assignment["family"] == "D" and assignment["step"] == "PRE":
                signature.append(
                    (
...
def compare(reference_rows: list[dict[str, Any]], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        comparisons.append(
            {
                ...
                "dpre_disagreement": dpre_signature(candidate) != dpre_signature(baseline),
            }
        )
    ...
    return {
        ...
        "dpre_disagreements": sum(row["dpre_disagreement"] for row in comparisons),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-level", type=int, default=21)
    parser.add_argument("--work-dir", type=pathlib.Path, default=WORK_DIR)
    parser.add_argument("--search-seed-base", type=int, default=225_121_000)
    parser.add_argument("--holdout-seed-base", type=int, default=225_121_999)
...
def main() -> int:
    ...
    for split in phases:
        ...
        print(
            ...
            f"DPRE disagreements={summary['dpre_disagreements']}"
        )
