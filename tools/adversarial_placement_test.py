#!/usr/bin/env python3
"""Search prefill-placement disagreements and audit a separately seeded split."""
...
SOURCE = ROOT / "scheduler_versions" / "v22_placement_experiment.cpp"
JUDGE = ROOT / "tools" / "local_judge.py"
WORK_DIR = ROOT / "build" / "adversarial-placement"
MODES = (
    "pair_gain",
    "short_partner",
    "long_pair",
...
def make_scenario(seed: int, split: str, index: int) -> dict[str, Any]:
    ...
    data["name"] = f"placement_{split}_{index:03d}_{mode}"
    data["description"] = (
        f"Deterministic placement adversarial {split} case; mode={mode}, seed={seed}."
    )

    system = data["system"]
    system["K"] = rng.choice([2, 2, 3, 4])
    system["S"] = round(rng.uniform(0.2, 10.0), 9)
    system["latency_in_ms"] = round(
        rng.uniform(0.01, 8.0) if mode == "latency_mix" else rng.uniform(0.001, 0.8), 9
    )
    system["bandwidth_gbps"] = round(rng.uniform(1.0, 100.0), 9)
    system["bytes_per_token"] = rng.choice([64, 128, 256, 512, 1024, 2048])

    schedule_cost = float(system["S"])
    singleton_proc = rng.uniform(8.0, 60.0)
    efficiency = rng.uniform(3.01, 3.2) if mode == "near_threshold" else rng.uniform(3.5, 8.0)
    pair_proc = max(0.001, 2.0 * (schedule_cost + singleton_proc) / efficiency - schedule_cost)
    pre_base = rng.uniform(0.05, 5.0)
    post_base = rng.uniform(0.05, 5.0)
    task_times = []
    for size in BATCH_SIZES:
        if size == 1:
            ...
            proc = pair_proc * (1.0 + 0.18 * math.sqrt(size - 2))
        post = post_base * (0.7 + 0.3 * math.sqrt(size))
        if mode == "post_cliff" and size >= 2:
            post *= rng.uniform(3.0, 9.0)
        task_times.append(
            {
                "batch_size": size,
                "prefill_pre": round(0.1 + 0.02 * math.sqrt(size), 9),
                "prefill_proc": round(
                    rng.uniform(0.2, 2.0) * (1.0 + 0.03 * math.sqrt(size)), 9
                ),
                "prefill_post": round(0.1 + 0.02 * math.sqrt(size), 9),
                "decode_pre": round(pre_base * (0.7 + 0.3 * math.sqrt(size)), 9),
                "decode_proc": round(proc, 9),
                "decode_post": round(post, 9),
            }
        )
    data["task_times"] = task_times

    request_count = rng.choice([4, 6, 8, 12, 16, 24])
    initial_wave = rng.choice([2, 3, 4, min(8, request_count)])
    gap = rng.uniform(5.0, 120.0)
    requests = []
    for request_id in range(request_count):
        if request_id < initial_wave:
            arrival = 0.0
        elif mode == "late_wave":
            arrival = gap * (1 + (request_id - initial_wave) // max(1, initial_wave))
        else:
            arrival = rng.uniform(0.0, gap)
        if mode == "short_partner" and request_id % 2 == 1:
            output_length = rng.choice([1, 1, 2])
        elif mode == "long_pair" and request_id < initial_wave:
            output_length = rng.choice([32, 48, 64])
        else:
            output_length = rng.choice([1, 2, 4, 8, 12, 16, 24, 32, 48])
        requests.append(
            {
                "arrival": round(arrival, 9),
                "input_length": (
                    rng.choice([1, 2, 4, 8, 16])
                    if mode == "prefill_skew" else rng.choice([1, 2, 4])
                ),
    ...
    pre_rate = max(
        size / (schedule_cost + task_times[BATCH_SIZES.index(size)]["decode_pre"])
        for size in eligible
    )
    proc_rate = max(
        size / (schedule_cost + task_times[BATCH_SIZES.index(size)]["decode_proc"])
        for size in eligible
    )
    post_rate = max(
        size / (schedule_cost + task_times[BATCH_SIZES.index(size)]["decode_post"])
        for size in eligible
    )
    ...
    singleton_path = 3.0 * schedule_cost + pre_base + singleton_proc + post_base + 2.0 * float(system["latency_in_ms"])
    throughput_weight = rng.choice([0.6, 0.8, 0.95, 1.0])
    scoring = data["scoring"]
    scoring["SLO1"] = round(max(float(scoring["SLO1"]), singleton_path * rng.uniform(2.0, 20.0)), 9)
...
def placement_signature(row: dict[str, Any]) -> list[tuple[float, int | None, tuple[int, ...]]]:
    signature = []
...
def compare(reference_rows: list[dict[str, Any]], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        comparisons.append(
            {
                ...
                "placement_disagreement": placement_signature(candidate) != placement_signature(baseline),
            }
        )
    ...
    return {
        ...
        "placement_disagreements": sum(row["placement_disagreement"] for row in comparisons),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-level", type=int, default=22)
    parser.add_argument("--work-dir", type=pathlib.Path, default=WORK_DIR)
    parser.add_argument("--search-seed-base", type=int, default=225_220_000)
    parser.add_argument("--holdout-seed-base", type=int, default=225_229_999)
...
def main() -> int:
    ...
    for split in phases:
        ...
        print(
            ...
            f"placement disagreements={summary['placement_disagreements']}"
        )
