#!/usr/bin/env python3
"""Search for D POST policy disagreements and audit a separate sealed holdout split."""
...
WORK_DIR = ROOT / "build" / "adversarial-dpost"
MODES = (
    "smooth_amortized",
    "latency_tension",
    "medium_knee",
...
def make_scenario(seed: int, split: str, index: int) -> dict[str, Any]:
    ...
    data["name"] = f"dpost_{split}_{index:03d}_{mode}"
    data["description"] = (
        f"Deterministic D POST adversarial {split} case; mode={mode}, seed={seed}."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--search-count", type=int, default=48)
    parser.add_argument("--holdout-count", type=int, default=24)
    parser.add_argument("--reference-level", type=int, default=15)
    parser.add_argument("--candidate-level", type=int, default=19)
    parser.add_argument("--work-dir", type=pathlib.Path, default=WORK_DIR)
    parser.add_argument("--search-seed-base", type=int, default=225_119_000)
    parser.add_argument("--holdout-seed-base", type=int, default=225_119_999)
    parser.add_argument(
        "--json-out",
        type=pathlib.Path,
        default=None,
