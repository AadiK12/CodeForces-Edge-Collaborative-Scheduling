#!/usr/bin/env python3
"""Search burst stage-ordering disagreements and audit a sealed holdout split."""
...
SOURCE = ROOT / "scheduler_versions" / "v24_burst_fifo_experiment.cpp"
JUDGE = ROOT / "tools" / "local_judge.py"
WORK_DIR = ROOT / "build" / "adversarial-burst-fifo"
MODES = (
    "edge_amortized",
    "staggered_tail",
    "slow_link",
...
def stage_curve(rng: random.Random, mode: str, stage: str) -> list[float]:
    ...
    for size in BATCH_SIZES:
        ...
        if stage == "pre" and mode in {"finite_remainder", "pipeline_shift"}:
            scale = 0.7 + 0.22 * size**0.55
            if size > knee:
                scale += rng.uniform(0.05, 0.4) * (size - knee) / knee
        elif stage == "pre" and mode == "pre_cliff" and size >= knee:
...
def make_scenario(seed: int, split: str, index: int) -> dict[str, Any]:
    ...
    data["name"] = f"burst_fifo_{split}_{index:03d}_{mode}"
    data["description"] = (
        f"Deterministic burst-ordering adversarial {split} case; mode={mode}, seed={seed}."
    )

    system = data["system"]
    system["K"] = 1
    system["S"] = round(rng.uniform(0.1, 10.0), 9)
    system["latency_in_ms"] = round(rng.uniform(0.05, 8.0), 9)
    system["bandwidth_gbps"] = round(
        math.exp(rng.uniform(math.log(0.005), math.log(0.09))), 9
    )
    system["bytes_per_token"] = rng.randint(1_000, 120_000)

    schedule_cost = float(system["S"])
    for row in data["task_times"]:
        if mode == "prefill_heavy":
            row["prefill_pre"] = round(float(row["prefill_pre"]) * rng.uniform(2.0, 8.0), 9)
            row["prefill_proc"] = round(float(row["prefill_proc"]) * rng.uniform(2.0, 8.0), 9)
            row["prefill_post"] = round(float(row["prefill_post"]) * rng.uniform(2.0, 8.0), 9)
        if mode == "decode_heavy":
            row["decode_pre"] = round(float(row["decode_pre"]) * rng.uniform(2.0, 8.0), 9)
            row["decode_proc"] = round(float(row["decode_proc"]) * rng.uniform(2.0, 8.0), 9)
            row["decode_post"] = round(float(row["decode_post"]) * rng.uniform(2.0, 8.0), 9)
        if mode == "nonmonotonic" and row["batch_size"] in {4, 16}:
            row["decode_pre"] = round(float(row["decode_pre"]) * rng.uniform(0.25, 0.6), 9)
            row["decode_post"] = round(float(row["decode_post"]) * rng.uniform(2.5, 7.0), 9)

    requests = data["requests"]
    while len(requests) < 8:
        requests.append(dict(requests[-1]))
    initial = len(requests) if mode != "staggered_tail" else max(8, len(requests) // 2)
    for request_id, request in enumerate(requests):
        request["arrival"] = (
            0.0 if request_id < initial else round(rng.uniform(0.1, 80.0), 9)
        )
        request["input_length"] = rng.choice([1, 2, 4, 8, 16])
        if mode == "output_skew" and request_id % 3 == 0:
            request["output_length"] = rng.choice([1, 2, 48, 64])
        else:
            request["output_length"] = rng.choice([2, 4, 8, 12, 16, 24, 32])
    requests.sort(key=lambda request: request["arrival"])

    request_count = len(requests)
    eligible = [size for size in BATCH_SIZES if size <= request_count]
    pre_rate = max(
        size / (schedule_cost + data["task_times"][BATCH_SIZES.index(size)]["decode_pre"])
        for size in eligible
    )
    proc_rate = max(
        size / (schedule_cost + data["task_times"][BATCH_SIZES.index(size)]["decode_proc"])
        for size in eligible
    )
    post_rate = max(
        size / (schedule_cost + data["task_times"][BATCH_SIZES.index(size)]["decode_post"])
        for size in eligible
    )
    link_rate = float(system["bandwidth_gbps"]) * 1_000_000.0 / max(
        8.0 * int(system["bytes_per_token"]), 1.0
    )
    estimated_rate = max(1e-10, min(pre_rate, proc_rate, post_rate, link_rate))
    singleton_path = (
        3.0 * schedule_cost +
        sum(data["task_times"][0][column] for column in ("decode_pre", "decode_proc", "decode_post")) +
        2.0 * float(system["latency_in_ms"])
    )
    throughput_weight = rng.choice([0.15, 0.2, 0.25, 0.3, 0.4])
    scoring = data["scoring"]
    scoring["SLO1"] = round(max(1.0, singleton_path * rng.uniform(2.0, 20.0)), 9)
    scoring["SLO2"] = round(max(0.001, singleton_path * rng.uniform(0.5, 4.0)), 9)
...
def action_signature(row: dict[str, Any]) -> list[tuple[float, str, str, str, tuple[int, ...]]]:
    signature = []
    for frame in row.get("assignment_trace", []):
        for assignment in frame["assignments"]:
            signature.append((round(float(frame["time"]), 9), assignment["server"], assignment["family"], assignment["step"], tuple(assignment["members"])))
    return signature
...
def compare(reference_rows: list[dict[str, Any]], candidate_rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        comparisons.append(
            {
                ...
                "action_disagreement": action_signature(candidate) != action_signature(baseline),
            }
        )
    ...
    return {
        ...
        "action_disagreements": sum(row["action_disagreement"] for row in comparisons),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-level", type=int, default=24)
    parser.add_argument("--work-dir", type=pathlib.Path, default=WORK_DIR)
    parser.add_argument("--search-seed-base", type=int, default=225_240_000)
    parser.add_argument("--holdout-seed-base", type=int, default=225_249_999)
...
def main() -> int:
    ...
    for split in phases:
        ...
        print(
            ...
            f"action disagreements={summary['action_disagreements']}"
        )
