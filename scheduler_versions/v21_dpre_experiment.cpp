...
static_assert(1 <= OPT_LEVEL && OPT_LEVEL <= 21, "OPT_LEVEL must be in [1, 21]");
...
// SUBMISSION_FEATURE_END terminal_dproc
// SUBMISSION_FEATURE_BEGIN terminal_dpre
constexpr bool kTerminalDPreOptimizer = OPT_LEVEL >= 21;
// SUBMISSION_FEATURE_END terminal_dpre
...
class LayeredScheduler {
  ...
  private:
    ...
    // SUBMISSION_FEATURE_BEGIN terminal_dpre
    vector<int> terminal_dpre_members() {
        vector<int> fallback = legacy_d_pre_members();
        vector<int> ready = collect_ready(d_pre_ready_, RequestState::READY_D_PRE);
        const int available = static_cast<int>(ready.size());
        const int fallback_size = static_cast<int>(fallback.size());
        if (available <= 1 || cloud_count_ != 1 ||
            throughput_weight_ < 0.95 || completed_output_lengths_.size() < 6 ||
            total_active_decode_requests_ > 9 || fallback_size < 4 ||
            completed_output_lengths_.size() >=
                2 * static_cast<size_t>(total_active_decode_requests_)) {
            return fallback;
        }

        set<int> sizes = candidate_group_sizes(available);
        sizes.insert(fallback_size);
        for (int size : {
                 2 * fallback_size,
                 available / 2,
                 3 * available / 4,
        ...
        auto path_cost = [&](int size) {
            const long long bytes = static_cast<long long>(size) * bytes_per_token_;
            return (3.0 * schedule_cost_ +
                    duration(DurationColumn::DECODE_PRE, size) +
                    duration(DurationColumn::DECODE_PROC, size) +
                    duration(DurationColumn::DECODE_POST, size) +
                    2.0 * transfer_time(bytes)) / size;
        };
        auto bottleneck_cost = [&](int size) {
            const long long bytes = static_cast<long long>(size) * bytes_per_token_;
            return max({
                (schedule_cost_ + duration(DurationColumn::DECODE_PRE, size)) / size,
                (schedule_cost_ + duration(DurationColumn::DECODE_PROC, size)) / size,
                (schedule_cost_ + duration(DurationColumn::DECODE_POST, size)) / size,
                transfer_time(bytes) / size,
            });
        };
        const double fallback_path_cost = path_cost(fallback_size);
        const double fallback_bottleneck_cost = bottleneck_cost(fallback_size);
        const double fallback_proc_cost =
            (schedule_cost_ + duration(DurationColumn::DECODE_PROC, fallback_size)) /
            fallback_size;
        const double fallback_post_cost =
            (schedule_cost_ + duration(DurationColumn::DECODE_POST, fallback_size)) /
            fallback_size;
        if (min(fallback_proc_cost, fallback_post_cost) <
            0.65 * fallback_bottleneck_cost - 1e-12) {
            return fallback;
        }
        int best_size = fallback_size;
        double best_bottleneck_cost = fallback_bottleneck_cost;
        double best_path_cost = fallback_path_cost;
        for (int size : sizes) {
            if (size < fallback_size || 4 * size > 5 * fallback_size) {
                continue;
            }
            const double candidate_path_cost = path_cost(size);
            const double candidate_bottleneck_cost = bottleneck_cost(size);
            if (candidate_path_cost > fallback_path_cost + 1e-12 ||
                candidate_bottleneck_cost > 0.90 * fallback_bottleneck_cost + 1e-12 ||
                downstream_group_is_hostile(size)) {
                continue;
            }
            if (candidate_bottleneck_cost < best_bottleneck_cost - 1e-12 ||
                (abs(candidate_bottleneck_cost - best_bottleneck_cost) <= 1e-12 &&
                 candidate_path_cost < best_path_cost - 1e-12)) {
                best_bottleneck_cost = candidate_bottleneck_cost;
                best_path_cost = candidate_path_cost;
                best_size = size;
            }
        }
        if (best_size == fallback_size) {
            return fallback;
        }
        ready.resize(best_size);
        return ready;
    }
    // SUBMISSION_FEATURE_END terminal_dpre
    ...
    vector<int> choose_d_pre_members() {
        // SUBMISSION_FEATURE_BEGIN terminal_dpre
        if constexpr (kTerminalDPreOptimizer) {
            return terminal_dpre_members();
        }
        // SUBMISSION_FEATURE_END terminal_dpre
        // SUBMISSION_FEATURE_BEGIN experimental_grouping
        if constexpr (kExperimentalGrouping) {
