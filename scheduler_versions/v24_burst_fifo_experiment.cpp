...
static_assert(1 <= OPT_LEVEL && OPT_LEVEL <= 24, "OPT_LEVEL must be in [1, 24]");
...
class LayeredScheduler {
  ...
  private:
    ...
    bool use_burst_fifo_ordering() const {
        if constexpr (kOptimizationLevel < 24) {
            return false;
        }
        if (cloud_count_ != 1 || throughput_weight_ < 0.15 ||
            throughput_weight_ > 0.4) {
            return false;
        }
        int reserved = 0;
        ...
        return reserved + static_cast<int>(p_pre_ready_.size()) >= 8;
    }
    ...
    vector<Candidate> edge_candidates() {
        ...
        if constexpr ((kOptimizationLevel >= 7 && kOptimizationLevel < 11) ||
            ...
            if (link_constrained &&
                (kOptimizationLevel < 11 || use_burst_fifo_ordering())) {
                stable_sort(
                    candidates.begin(),
                    candidates.end(),
                    [](const Candidate& left, const Candidate& right) {
